...
create_img_from_base()
{
  ...
  do_on_exit umount $LOOP_DIR
  
  execute "mount -t proc none $LOOP_DIR/proc"
  do_on_exit umount $LOOP_DIR/proc
...
create_strongswan_img()
{
  STRONGSWAN_IMG="${DIR}/rootfs/rootfs_strongswan.${IMG_EXT}"
  BASE_IMG="${DIR}/rootfs/rootfs_debian_amd64.${IMG_EXT}"

  create_img_from_base ${STRONGSWAN_IMG} ${BASE_IMG}
